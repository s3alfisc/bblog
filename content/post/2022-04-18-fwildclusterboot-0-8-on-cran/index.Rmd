...
By default, `boottest()` will run `fwildclusterboot's` R-implementation of the wild cluster bootstrap. To run the bootstrap through `WildBootTests.jl`, you have to specify the `boot_algo` function argument: 
...
In a blog post from a few months ago, I claimed that [`fwildclusterboot` was 1000 times faster than `sandwich's` `vcovBS()` function](https://s3alfisc.github.io/blog/post/1000x-faster-wild-cluster-bootstrap-inference-in-r-with-fwildclusterboot/). Below, I run the same problem, once for `boot_algo = "R"` and once with `boot_algo = "WildBootTests.jl"`:
...
Afterwards, each call to `boottest()` will simply default to `boot_algo = "WildBootTests.jl"` unless explicitly stated otherwise. 
